...
> [!NOTE]
> **Development Environment:** Active local development on **Linux** with **Java 26** and **Gradle 9.6.1**.<br>
> **Production Compatibility:** The Gradle toolchain targets **Java 25**, so release builds must remain Java 25 compatible.<br>
> **Default and Development Branch:** `26.2`<br>
> **Paper Target:** `26.2.build.+`<br>
> **Runtime Goal:** Keep this fork compatible with the Paper **26.2.x** release line for as long as possible.<br>
...
| Original dynmap Repository                          | This fork (`26.2` branch)                                                                    |
|:----------------------------------------------------|:--------------------------------------------------------------------------------------------|
...
| **Build System:** Outdated Gradle build             | **Build System:** Upgraded to **Gradle 9.6.1**                                              |
...
Database storage classes for MySQL and MariaDB are present. The release resources include MariaDB Connector/J 3.5.9, which is extracted to `drivers/mariadb-java-client-3.5.9.jar`. Dynmap first checks whether the configured driver is already available on the server classpath. Otherwise, configure `storage/driver-jar` and optionally `storage/driver-class`; the same mechanism can load a separately supplied MySQL Connector/J.
